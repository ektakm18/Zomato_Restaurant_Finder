    ...
    methods: {
        searchRestro: function () {
            if(this.searchedRestro.trim()){
                const vm = this
                this.isLoading = true
                axios.get(`http://localhost:8080/api/search?q=${this.searchedRestro}`)
                .then(response => {
                    if(response == null || response.data.restaurants.length==0){
                        vm.isFound = true;                            
                    }else{
                        socket.emit('history-added', this.searchedRestro)
                        vm.isLoading = false                  
        ...
        showFromHistory: function (item) {
            ...
            this.searchedRestro = item;
            this.searchRestro();
          /*   const vm = this
             axios.get(`http://localhost:8080/api/search?q=${item}`)
             .then(response => {
                 if(response == null || response.data.results_found==0){
                     vm.isFound = true;
                 }else{
                     vm.isLoading = false
                     vm.restaurants=response.data.restaurants
...
socket.on('newhistorylist', (historyList) => {
    zomato.searchHistory = historyList
})
